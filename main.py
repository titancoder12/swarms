...
class Boid:
    ...
    def update(self, blocks, WIDTH, HEIGHT):
        ...
        for block in blocks:
            ...
            boid_rect = pygame.Rect(self.position.x, self.position.y, 5, 5)
    ...
    def resolve_collision_with_ball(self, objects):
        for ball in objects:
            ...
            overlap = ball.size + 5 - distance
            if overlap > 0:
                push_dir = (self.position - ball.position).normalize()
                self.position += push_dir * overlap
                self.velocity.reflect_ip(push_dir)
                ball.apply_force(-push_dir * 0.5)

    def flock(self, boids, blocks, objects=None, target_position=None):
        ...
        if objects and target_position:
...
def render_UI(screen, boids):
    ...
    a, b, c, d = 140, 10, 50, 10

    button_add_boids = pygame.Rect(a, b, c, d)
    button_remove_boids = pygame.Rect(a+60, b, c, d)
    button_add_speed = pygame.Rect(a, b+20, c, d)
    button_remove_speed = pygame.Rect(a+60, b+20, c, d)
    button_add_force = pygame.Rect(a, b+40, c, d)
    ...
    buttons = [button_add_boids, button_remove_boids, button_add_speed, button_remove_speed,
               button_add_force, button_remove_force, button_add_neighbor_radius, button_remove_neighbor_radius,
               button_add_separation_radius, button_remove_separation_radius, 
               button_add_object_separation_radius, button_remove_object_separation_radius]

    for button in buttons:
        pygame.draw.rect(screen, (255, 255, 255), button)

    for button, label in [
        (button_add_boids, "+"), (button_remove_boids, "-"),
        (button_add_speed, "+"), (button_remove_speed, "-"),
        (button_add_force, "+"), (button_remove_force, "-"),
        (button_add_neighbor_radius, "+"), (button_remove_neighbor_radius, "-"),
        (button_add_separation_radius, "+"), (button_remove_separation_radius, "-"),
        (button_add_object_separation_radius, "+"), (button_remove_object_separation_radius, "-")
    ]:
    ...
    # Display stats
    boid_count_text = font.render(f"Boids: {len(boids)}", True, (255, 255, 255))
...
def manage_UI(buttons, boids, objects=None):
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE:
            return "menu"  # Return to menu when escape is pressed
    ...
    if mouse_held and current_time - last_add_time > 50:
...
def show_menu(screen, font):
    ...
    title_text = font.render("Swarm Simulation", True, (255, 255, 255))
    title_rect = title_text.get_rect(center=(WIDTH//2, HEIGHT//2 - 100))
    screen.blit(title_text, title_rect)
    
    # Add escape instruction
    escape_text = font.render("Press ESC to return to menu from simulation", True, (128, 128, 128))
    escape_rect = escape_text.get_rect(center=(WIDTH//2, HEIGHT//2 - 70))
    screen.blit(escape_text, escape_rect)
    
    button_width, button_height = 200, 50
    button1 = pygame.Rect(WIDTH//2 - button_width//2, HEIGHT//2 - 25, button_width, button_height)
    button2 = pygame.Rect(WIDTH//2 - button_width//2, HEIGHT//2 + 50, button_width, button_height)
    
    pygame.draw.rect(screen, (100, 100, 100), button1)
    pygame.draw.rect(screen, (100, 100, 100), button2)
    
    text1 = font.render("Swarm Original", True, (255, 255, 255))
    text2 = font.render("Swarm Goal", True, (255, 255, 255))
...
def run_swarm_original():
    global NUM_BOIDS, WIDTH, HEIGHT, OBJECTS_IN_GOAL
    
    pygame.init()
    screen = pygame.display.set_mode((WIDTH, HEIGHT), pygame.RESIZABLE)
    pygame.display.set_caption("Swarm Original - Press ESC to return to menu")
    ...
    while running:
        ...
        result = manage_UI(buttons, boids)
...
def run_swarm_goal():
    global NUM_BOIDS, WIDTH, HEIGHT, OBJECTS_IN_GOAL, target_start_time
    
    pygame.init()
    screen = pygame.display.set_mode((WIDTH, HEIGHT), pygame.RESIZABLE)
    pygame.display.set_caption("Swarm Goal - Press ESC to return to menu")
    clock = pygame.time.Clock()
    ...
    while running:
        ...
        buttons = render_UI(screen, boids)
        result = manage_UI(buttons, boids, objects)
        ...
        # Check if all objects are in target
...
def main():
    ...
    pygame.display.set_caption("Swarm Simulation - Menu")
    font = pygame.font.SysFont(None, 36)
    clock = pygame.time.Clock()

    running = True
    while running:
        button1, button2 = show_menu(screen, font)
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if button1.collidepoint(event.pos):
                    result = run_swarm_original()
                    if result == "quit":
                        running = False
                    # If result == "menu", continue the loop to show menu again
                elif button2.collidepoint(event.pos):
                    result = run_swarm_goal()
                    if result == "quit":
                        running = False
                    # If result == "menu", continue the loop to show menu again
